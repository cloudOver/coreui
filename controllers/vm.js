...
window.app.controller('VmEditCtrl', function ($scope, $location, $route, $routeParams) {
    ...
    $scope.vmConsole = function() {
        request('/api/vm/console/', {token: $.cookie('core_token'), vm_id: $route.current.params.id, enable: true}, function(){
            $('#noVNC').modal('toggle');
        });
    };
    ...
    request('/api/vm/get_by_id/', {token: $.cookie('core_token'), vm_id: $route.current.params.id}, function(vm) {
        ...
        $scope.webvnc_path = 'novnc/vnc.html#host=' + window.vncHost + '&port=' + window.vncPort + '&path=' + window.vncPrefix + '/' + $route.current.params.id + '/&password=' + vm.vnc_passwd;
